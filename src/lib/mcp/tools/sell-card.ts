...
export default defineTool({
  ...
  description:
    "Registra il prezzo di vendita di una carta: la carta passa nella sezione Plusvalenze.",
  inputSchema: {
  ...
  handler: async (input, ctx) => {
    ...
    if (matches.length === 0)
      throw new ToolError("Nessuna carta corrispondente trovata in galleria.");
    if (matches.length > 1)
      throw new ToolError(
        "Più carte corrispondono: specifica il seriale per identificarne una sola.",
      );
    ...
    return {
      content: [
        {
          type: "text",
          text: `Venduta ${updated.name} ${updated.season} a €${updated.sell!.toFixed(2)} — plusvalenza €${profit.toFixed(2)}`,
        },
      ],
